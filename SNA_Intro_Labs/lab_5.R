...
saveRDS(twitter_token, "SNA_Intro_Labs/rtweet.rds")
...
# TWITTER_PAT=SNA_Intro_Labs/rtweet.rds 
...
?search_tweets
davos_tweets <- search_tweets(q =  "#Davos2020",
                               type = 'recent',
                               lang="en",
...
saveRDS(davos_tweets, 'data/davos_tweets_21-01-2020.RData')
# load the saved data (for offline work)
# davos_tweets <- readRDS("data/davos_tweets_21-01-2020.RData")

# Get a glimpse into the obtained dataset
glimpse(davos_tweets)
...
davos_tweets_users <- davos_tweets %>%
...
?unnest
davos_tweets_users <- unnest(davos_tweets_users, mentioned) 
View(davos_tweets_users[1:20,])

# To create graphs, we need to transform our davos_tweets_users dataframe
...
replied_to_edgelist <- davos_tweets_users %>%
  ...
  summarise(weight = n()) %>%         # compute the size of each group (i.e. number of connections 
                                      # between sender and replied_to nodes) and assign it to the 
                                      # 'weight' variable
  ungroup()
...
mentioned_edgelist <- davos_tweets_users %>%
  select(-replied_to) %>%
...
# check that we really got a list of unique users
n_distinct(reply_to_unique) == length(reply_to_unique)

# Unique users in mentioned edgelist
mention_unique <- union(mentioned_edgelist$sender, mentioned_edgelist$mentioned)
n_distinct(mention_unique) == length(mention_unique)
...
# in the dataset we have already collected from Twiter (davos_tweets)
glimpse(davos_tweets)
...
# save the data
saveRDS(alters_data, "data/davos_raw_alters_data_21-01-2020.RData")
# load the saved data (for offline work)
# alters_data <- readRDS("data/davos_raw_alters_data_21-01-2020.RData")
...
mentioned_edgelist_reduced %>%
  ...
  saveRDS(file = "data/mentions_edgelist_davos_21-01-2020.RData")

replied_to_edgelist_reduced %>%
  filter(!replied_to %in% missing_alter) %>%
  rename(ego=sender, alter=replied_to, reply_to_tie=weight) %>%
  saveRDS(file = "data/replied_to_edgelist_davos_21-01-2020.RData")

saveRDS(senders_data, file = "data/ego_data_davos_21-01-2020.RData")
saveRDS(alters_data, file = "data/alter_data_davos_21-01-2020.RData")
...
mention_edgelist <- readRDS("data/mentions_edgelist_davos_21-01-2020.RData")
...
senders_data <- readRDS("data/ego_data_davos_21-01-2020.RData")
# Extract the set of attributes we are interested in 
ego_attrs <- get_user_attrs(senders_data, V(mention_net)$name)
glimpse(ego_attrs)

# Load data and extract attributes about users mentioned in tweets (alters)
alters_data <- readRDS("data/alter_data_davos_21-01-2020.RData")
...
friends_for_color <- attr_based_color_gradient(log1p(V(mention_net)$friends_count), 
...
# Use the same kind of mapping between nodes' attributes (friends_count and followers_count)
#  and their visual representation (color and size, respectively)
gc_colors <- attr_based_color_gradient(log1p(V(giant_comp)$friends_count), 
                                       c('grey100','midnightblue'))
